...
of main relevance. Here are reported some of these words:
...
#### D. Dataset Split
The training validation, and test set are split from the dataset with ratio training:validation:test = 3:1:1. I decide not to apply a K–fold cross validation since the creation of the features explained in III-C is time consuming and I was not able to repeat the process K times. Moreover, the size of the data set permits to obtain informative result with just a single split.
...
The test error chosen is always F1-score for the same reasons specified above. The results are shown in the follwing Table.
